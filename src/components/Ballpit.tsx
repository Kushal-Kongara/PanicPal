...
// @ts-nocheck  // remove this if you want to strictly type everything later
...
interface SpheresConfig extends WConfig {
  colors: number[];
  ambientColor: number;
...
class X {
  ...
  constructor(config: ThreeXConfig) {
    this.#config = { ...config };
    this.#initCamera();
    this.#initScene();
    this.#initRenderer();
...
const DEFAULT_SPHERES_CONFIG: SpheresConfig = {
  count: 200,
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<SpheresConfig> = {}) {
    const config: SpheresConfig = { ...DEFAULT_SPHERES_CONFIG, ...params };

    // ❌ Removed RoomEnvironment/PMREM/envMap usage
...
function createBallpit(canvas: HTMLCanvasElement, config: Partial<SpheresConfig> = {}): CreateBallpitReturn {
